...
void WaveShare_EPD::SetFont(FONT fontindex)
{
  ...
  void WaveShare_EPD::DrawCircle(int x,int y,int r,bool fill)
  {
    if(fill=0)
    {
      for(int i=0; i<360; i++)
...
int WaveShare_EPD::getIcon(int weathercodeindex) {
 ...
 void WaveShare_EPD::DrawWeatherChart(int xmin,int xmax,int ymin,int ymax,int point_n,int line_n,String tmax,String tmin,String code_d,String code_n,String text_d,String text_n,String date,String week)//绘制天气温度变化曲线
 {
...
void WaveShare_EPD::DrawLine(int xstart,int ystart, int xend,int yend)
{
...
void WaveShare_EPD::Inverse(int xStart,int xEnd,int yStart,int yEnd)
{
...
void WaveShare_EPD::DrawXbm_p_gray(int16_t xMove, int16_t yMove, int16_t width, int16_t height,const unsigned char *xbm,byte level)
{
  ...
  for(int16_t x = 0; x < width; x++) {
    for(int16_t y = 0; y < height; y++ ) {
      ...
      // if there is a bit draw it
      if (((Data & 0x0F)==level)) {SetPixel(xMove + y, yMove + x);CurrentCursor=x;}
    }
  }
  
  }
void WaveShare_EPD::DrawXbm_spiff_gray(int16_t xMove, int16_t yMove, int16_t width, int16_t height,byte level)
{
...
void WaveShare_EPD::InversePixel(int16_t x, int16_t y)
{ 
...
unsigned char WaveShare_EPD::ReadBusy_long(void)
{
  unsigned long i=0;
  for(i=0;i<2000;i++){
  //  println("isEPD_BUSY = %d\r\n",isEPD_CS);
...
void WaveShare_EPD::EPD_WriteDispRam_Old(unsigned int XSize,unsigned int YSize,unsigned char *Dispbuff,unsigned int offset,byte label)
{
...
void WaveShare_EPD::EPD_Set_Contrast(byte vcom)
{   if(EPD_Type==OPM42)
  {
    EPD_WriteCMD(0x2C);       // 
...
void WaveShare_EPD::EPD_Dis_Part(int xStart,int xEnd,int yStart,int yEnd,unsigned char *DisBuffer,unsigned char Label)
{
